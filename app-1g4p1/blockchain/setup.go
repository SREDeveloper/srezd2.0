...
func CleanupUserData(sdk *fabsdk.FabricSDK) {
	...
	if err != nil {
		...
		keyStorePath = "/tmp/app-1g4p1-store"
		credentialStorePath = "/tmp/app-1g4p1-msp"
...
func (setup *FabricSetup) Initialize() error {
	...
	//channelRes, err := setup.org_nowResMgmt.QueryChannels(resmgmt.WithTargetEndpoints("peer1.org4.hf.srezd.io"))
	if err != nil {
	...
	if !channelHasInstall {
		...
		req := resmgmt.SaveChannelRequest{ChannelID: setup.ChannelID, ChannelConfigPath: os.Getenv("GOPATH") + "/src/github.com/srezd/app-1g4p1/fixtures/artifacts/org4.srezd.anchors.tx", 
...
func (setup *FabricSetup) InstallAndInstantiateCC() error {
	...
	ccHasInstall := false
	// 查询已经安装的CC
	ccInstalledRes, err := setup.org_nowResMgmt.QueryInstalledChaincodes(resmgmt.WithTargets(setup.orgTestPeernow))
	if err != nil {
	...
	ccInstantiatedRes, err := setup.org_nowResMgmt.QueryInstantiatedChaincodes(setup.ChannelID, resmgmt.WithTargets(setup.orgTestPeernow))
	...
	if !ccHasInstantiate {
		...
		if found {
			fmt.Println("org_nowResMgmt app-1g4p1 found:ture")
		}else{
			fmt.Println("org_nowResMgmt app-1g4p1 found:false")
		}
	} else {
	...
	fmt.Println("Chaincode app-1g4p1 Installation & Instantiation Successful")
	return nil
}
